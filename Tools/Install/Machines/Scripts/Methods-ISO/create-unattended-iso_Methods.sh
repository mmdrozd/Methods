...
startPath=ccarrollATjhuecon

pathToScript=$(dirname `realpath "$0"`)
# pathToScript=/home/methods/ubuntu-unattended-install-options/ubuntu-unattended/
# file names & paths
...
# read answer
pwd
cmd="(mkisofs -D -r -V "NETSON_UBUNTU" -cache-inodes -J -l -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -o $tmp/$new_iso_name . > /dev/null 2>&1) &"
echo "$cmd"
