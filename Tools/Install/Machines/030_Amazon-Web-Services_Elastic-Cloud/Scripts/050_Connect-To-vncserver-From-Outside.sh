...
# 0. Relaunch the vncserver on the remote machine via the `vncserver` command (once the above command connects)

# 0. Connect to it from the original machine via, e.g., remmina
